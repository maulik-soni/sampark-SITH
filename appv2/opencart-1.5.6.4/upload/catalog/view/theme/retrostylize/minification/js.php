<?php
    ...
    if (file_exists(dirname(__DIR__).DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'facebook'.DIRECTORY_SEPARATOR.$type.'.js'))//&& default_mtime == filemtime(dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'public/scripts'.DIRECTORY_SEPARATOR.$typejs.'.js')
    {
            if (!empty($_SERVER['HTTP_IF_MODIFIED_SINCE']) && strtotime($_SERVER['HTTP_IF_MODIFIED_SINCE']) == filemtime(dirname(__DIR__).DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'facebook'.DIRECTORY_SEPARATOR.$type.'.js')) {
                    header("HTTP/1.1 304 Not Modified");
                    exit;
            }

            readfile(dirname(__DIR__).DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'facebook'.DIRECTORY_SEPARATOR.$type.'.js');
    ...
    else
    {
        ...
        include_once(dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'sellegance/js'.DIRECTORY_SEPARATOR.'custom-script.js');
       ...
       // include_once(dirname(dirname(dirname(__FILE__))).DIRECTORY_SEPARATOR.'sellegance/js'.DIRECTORY_SEPARATOR.'jquery.impromptu.js');
		...
		$fp = @fopen(dirname(__DIR__).DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'facebook'.DIRECTORY_SEPARATOR.$type.'.js', 'w');
		@fwrite($fp, $js);
		@fclose($fp);  
    }
		$lastModified = filemtime(dirname(__DIR__).'/cache/facebook/'.$type.'.js');
